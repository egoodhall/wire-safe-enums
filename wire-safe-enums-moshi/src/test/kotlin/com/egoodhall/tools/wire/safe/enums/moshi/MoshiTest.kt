package com.egoodhall.tools.wire.safe.enums.moshi
...
class MoshiTest {
  ...
  inner class JsonSerialization {
    ...
    fun `it serializes known value to JSON`() {
      assertThatSerializedJson(TestEnum.A.wireSafe()).isEqualTo("\"A\"")
    }
    ...
    fun `it serializes known wrapped value to JSON`() {
      assertThatSerializedJson(TestWrapper(TestEnum.A.wireSafe())).isEqualTo("{\"field\":\"A\"}")
    }
    ...
    fun `it serializes unknown map key to JSON`() {
      assertThatSerializedJson(mapOf(UNKNOWN_VALUE to "field")).isEqualTo("{\"B\":\"field\"}")
    }

    @Test
    fun `it serializes known list element to JSON`() {
      assertThatSerializedJson(listOf(TestEnum.A.wireSafe())).isEqualTo("[\"A\"]")
    }
    ...
    fun `it serializes known set element to JSON`() {
      assertThatSerializedJson(setOf(TestEnum.A.wireSafe())).isEqualTo("[\"A\"]")
    }
